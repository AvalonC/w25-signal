...
开场：保留星星汇聚成标题、愿望与祝福的呈现，操作提示通过光效表达，不显示上下教学文字；操作说明仅供屏幕阅读器读取。主星升起后先闪烁，轻点会得到远方的回应；随后光圈向外扩散，按住星星一秒让光圈蓄满。短按、移开、切后台或失焦会取消本次蓄光，不累积进度。成功后主星飞向愿望环节。重新开始与重温也会经历这段短引导；键盘支持按住空格或回车，减少动态模式保留静态光圈与进度。三个愿望聚拢成伴星，再带光走进星路。声音可以主动开启，开关选择会在当前设备记住。

1. **同行**：从散落的星光中聚拢三个愿望，化为主星身边的三颗伴星。
2. **星路**：拖动主星或轻触远处，可先去棱镜，也可先去生日星盘；两处都能随时返回与重访。
   愿望离场与星路入场使用实际屏幕坐标交接；两颗伴星先去勾勒棱镜与星盘，再归主星。棱镜由光点织出折面，星盘由弧线闭合、刻度归位，路线随后展开。
3. **两份发现**：棱镜保持无常驻文字的色散，粉色停留后散开，点亮半段路。星盘对齐 **10 月 8 日** 后旋转汇光，光化为星点和线框，标记宝石的位置。成果与当前位置会保存。
   星盘外径保持不变，手机中央月份/日期改为清楚的两位数字，星座符号与指针同步放大；外围仅保留相邻值，避免细字拥挤。按 402×874、402×720、320×568 视口验证，402px 宽时核心读数约84/90px、星座约45px，最大12/31均完整显示；此为浏览器模拟，非iPhone真机验证。
   两股光由星盘实际圆心释放为有宽度的粒子流，旋入中心形成有暗隙的星云，再从星云内诞生宝石轮廓，不绘制两条连接线。已找到粉色则为粉色星云，否则为银白色。星云中飞出的星星归位后闪烁，可带镜头连续返回右侧星盘位置；减少动态时保留清楚的星云形成与短过渡。
   棱镜先以镜面侧影出现在前方，主星靠近后视角转向玻璃的三角折面；直线光束入射后才展开光谱并解锁操作。找到粉光后，光路从主星逐步绕过棱镜，勾出下一处星盘或宝石，沿路文案随光路出现。主星与远方轮廓的闪烁提示可触碰。返回时选关地图在镜头后方展开，棱镜缩回真实图标位置，主星落回同一光点，地图不重新挂载或重播入场；回到地图后继续保留指向下一目的地的光路。探索界面的操作文案仅保留给读屏与帮助面板。
4. **相遇**：颜色和日期都点亮后，待第二处发现的动画完成，粉星自动带入相遇，不再需要回到选关点击宝石。主星停在宝石上方，向下流动的星屑指向顶部切面；从上方拖入或轻触顶部切面才会完成注光，自动进场不会代替玩家放入。光沿顶面、肩部和底部切面渐次透亮，不再使用圆盘光晕；交接到旋转探索时位置与尺寸保持一致。转到三个不同角度并各稳定停留 880 ms，依次发现“蓝宝石”“天秤与生日”“最爱的粉色”；反复乱转不会累积通关。全部发现后轻触出口粉光，带愿望前往回声。
...
- `components/game/date-nebula.tsx` / `lib/date-nebula.ts`：两股星屑汇聚、星云形成与宝石诞生的可暂停时间轴。
