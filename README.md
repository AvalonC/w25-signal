...
2. **光谱**：无常驻文字或滑杆；拖动透明棱镜，观察连续光谱柔和展开。粉色光晕扩散后无文字转场进入下一章；键盘可用左右方向键。
3. **星轨**：只触碰前三颗星组成 W；2 和 5 自动以长短光延续，随后自动转场。长光移动并留下拖尾。
4. **纪念日**：双转盘到 **10 月 8 日**，星点汇聚成可旋转的宝石线框。箭头、旋转示意和三步进度提示操作；左右拖动或点击按钮，每次点亮一道祝福，从转动后停留至少 4.6 秒。最后散回星点并进入回声。蓝宝石是这份礼物指定的诞生纪念，不按月份自动推算宝石。
...
- `components/game/sapphire-scene.tsx` / `star-sapphire.tsx`：星点线框宝石、连续旋转、三句祝福与自动离场。
