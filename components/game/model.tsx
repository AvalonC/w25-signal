...
export function useBraceletDelivery({onGem,onScatter,paused,enabled,reduced,viewer,surface}:{
  ...
  const clock=useVisibleClock(departing&&!paused),phase=braceletPhase(clock,reduced);
  useEffect(()=>{
    if(!departing||paused||document.hidden||!phase.done||done.current)return;
    done.current=true;onGem({id:Date.now(),points:positions.current});
  },[departing,paused,phase.done,onGem]);
  const depart=()=>{
    if(!enabled||started.current||paused||document.hidden||!surface.current)return;
  ...
  return {departing,view,positions,clock,phase,depart};
}
export function BraceletDissolve({delivery,reduced}:{delivery:ReturnType<typeof useBraceletDelivery>;reduced:boolean}){
  return <>{delivery.departing&&delivery.view&&<BraceletStardust elapsed={delivery.clock} reduced={reduced} points={points} view={delivery.view}
    onPositions={value=>{delivery.positions.current=value;}}/>}
    {delivery.departing&&<output className="sr-only">手链正化作星光，请稍候。</output>}</>;
}
export function BraceletActions({loaded,paused,departing,enabled,onDeliver}:{loaded:boolean;paused:boolean;departing:boolean;enabled:boolean;onDeliver:()=>void}){
  ...
  return <div className="bracelet-actions" inert={!enabled||departing||paused} aria-hidden={!enabled||departing}>
    ...
    <button className="bracelet-send" disabled={!enabled||paused||departing} onClick={onDeliver}>礼物已在身边，读完这封信</button>
